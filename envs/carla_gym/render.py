...
class MapImage(object):
    ...
    def draw_road_map(self, map_surface, carla_world, carla_map, world_to_pixel, world_to_pixel_width):
        ...
        def lateral_shift(transform, shift):
            # TODO Gets called often, can we optimize??
...
class BirdeyeRender(object):
    ...
    """
        actor_poly_dict = {}
        for actor in filt:
            actor = self.world.get_actor(actor.id)
    ...
    def render_waypoints(self, surface, waypoints, world_to_pixel):
        ...
        corners = [world_to_pixel(p) for p in corners]
        pygame.draw.lines(surface, color, False, corners, 1)
        for corner in corners:
            pygame.draw.circle(surface, color, (corner[0], corner[1]), 2, 0)
